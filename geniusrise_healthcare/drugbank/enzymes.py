...
def process_enzymes_file(enzymes_file: str, G: nx.DiGraph) -> None:
    """
    Processes the DrugBank enzymes data and adds it to the graph.

    Args:
        enzymes_file (str): Path to the DrugBank XML file.
        G (nx.DiGraph): The NetworkX graph to which the enzyme data will be added.

    Returns:
        None
    """
    log.info(f"Loading enzymes from {enzymes_file}")

    tree = read_xml_file(enzymes_file)
            ...
            if enzymes is not None:
                for enzyme in enzymes.findall("enzyme"):
                    ...
                    if enzyme_id:
                        ...
                        G.add_edge(drugbank_id, enzyme_id, type="enzymes")
        except Exception as e:
            log.error(f"Error processing enzyme for drug {drug}: {e}")
            raise ValueError(f"Error processing enzyme for drug {drug}: {e}")
